...
// can be added or removed over time; in other words, arrays are mutable.
...
// as an argument. Modify the function by using push() and unshift()
// to add 'I', 2, 'three' to the beginning of the array and 7, 'VIII', 9 to the end
...
console.error("Line 101 // Basic Data Structures: Add Items to an Array with push() and unshift()")
...
// opposites: pop() and shift(). As you may have guessed by now,
...
// between pop() and shift() and their cousins push() and unshift(), is that
// neither method takes parameters, and each only allows an array to be modified
// by a single element at a time.
//
// Let's take a look:
//
...
// elements to their corresponding variables, so that the returned array contains
// their values.
...
// do not change code below this line
console.warn("Line 145 // Basic Data Structures: Remove Items from an Array with pop() and shift()")
...
// array, we would use 0. splice()'s first parameter represents the index on the
// array from which to begin removing elements, while the second parameter
...
//
// We've defined a function, sumOfTen, which takes an array as an argument and
// returns the sum of that array's elements. Modify the function, using splice(),
...
// do not change code below this line
console.error("Line 188 // Basic Data Structures: Remove Items Using splice()");
...
// three parameters? Well, we can go one step further with splice() — in addition
// to removing elements, we can use that third parameter, which represents one
// or more elements, to add them as well. This can be incredibly useful for
...
// in an array, and want to dynamically change a color based on some action:
//
...
//
// This function takes an array of hex values, an index at which to remove an
// element, and the new color to replace the removed element with. The return value
// is an array containing a newly modified color scheme! While this example is a
// bit oversimplified, we can see the value that utilizing splice() to its maximum
// potential can have.
//
// We have defined a function, htmlColorNames, which takes an array of HTML colors
// as an argument. Modify the function using splice() to remove the first
...
console.warn("Line 234 // Basic Data Structures: Add Items Using splice()");
...
// at which to stop extraction (extraction will occur up to, but not including the
...
//
// In effect, we have created a new array by extracting elements
...
// do not change code below this line
console.error("Line 280 // Basic Data Structures: Copy Array Items Using slice()");
...
// copy all of an array's elements, in order, with a simple and highly
...
// so that it works correctly (hint: another method we have already covered
...
// copyMachine([true, false, true], 2) should return
// [[true, false, true], [true, false, true]]

// change code here to test different cases:

console.warn("Line 316 // Basic Data Structures: Copy an Array with the Spread Operator");
...
console.error("Line 356 // Basic Data Structures: Combine Arrays with the Spread Operator");
...
// indexOf(), that allows us to quickly and easily check for the presence of
// an element on an array. indexOf() takes an element as a parameter,
// and when called, it returns the position, or index, of that element, or -1
...
// an element on an array. We have defined a function, quickCheck, that takes
// an array and an element as arguments. Modify the function using indexOf()
...
console.warn("Line 388 // Basic Data Structures: Check For The Presence of an Element With indexOf()");
...
// In this way, we have easily and programmatically determined which data items
...
// passed array such that any array nested within arr containing elem
// has been removed.
...
// I spent significant time solving this problem. At first I was making use
// of a nested for loop [i], [j]. I was also making a separate variable to
// test the if statement with .indexOf instead of including it directly
// within it.

// change code here to test different cases:

console.error("Line 431 // Basic Data Structures: Iterate Through All an Array's Items Using For Loops");
console.log(filteredArray([[9, 2, 5], [1, 6, 3], [3, 13, 26], [19, 3, 9]], 3));
...
// level overview, and there is plenty more to learn about working with arrays,
// much of which you will see in later sections. But before moving on to looking
// at Objects, lets take one more look, and see how arrays can become a bit more
...
// contain other arrays, each with their own arbitrary levels of depth, and so on.
...
// and on the fifth level, include the string 'deepest'.
  ...
  let myNestedArray = [                                   //level 1
    ['unshift', false, 1, 2, 3, 'complex',                //level 2
      ['loop', 'shift', 6, 7, 'deep',                     //level 3
        ['concat', false, true, 'deeper',                 //level 4
          ['mutate', 1327.98, 'splice', 'deepest', ]      //level 5
        ]                                                 //close 4
      ]                                                   //close 3
    ]                                                     //close 2
  ];                                                      //close 1
...
console.warn("Line 492 // Basic Data Structures: Create complex multi-dimensional arrays");
console.log(myNestedArray[0][6][5][4][3]);
...
// or in other words, pieces of data mapped to unique identifiers that we call
// properties or keys. Let's take a look at a very simple example:
//
// let FCC_User = {
...
// have attempted to evaluate it as a variable, and a ReferenceError:
...
// bananas with a value of 13, grapes with a value of 35, and strawberries
// with a value of 27.
...
console.error("Line 563 // Basic Data Structures: Add Key-Value Pairs to JavaScript Objects");
console.log(foods);
...
//   }
// };
// nestedObject has three unique keys: id, whose value is a number, date whose
// value is a string, and data, whose value is an object which has yet another
// object nested within it. While structures can quickly become complex,
...
// nested within it. You can modify properties on this nested object in the
// same way you modified properties in the last challenge. Set the value of
...
console.warn("Line 618 // Basic Data Structures: Modify an Object Nested Within an Object");
...
// to check our foods object for the presence of that food. This might look like:
//
// let selectedFood = getCurrentFood(scannedItem);
// let inventory = foods[selectedFood];
//
// This code will evaluate the value stored in the selectedFood variable and
// return the value of that key in the foods object, or undefined if
// it is not present. Bracket notation is very useful because sometimes object
// properties are not known before runtime or we need to access them in
// a more dynamic way.
//
//
...
console.warn("Line 665 // Basic Data Structures: Access Property Names with Bracket Notation");
...
// In short, they are key-value stores which provide a flexible, intuitive way
// to structure data, and, they provide very fast lookup time.
// Throughout the rest of these challenges, we will describe several common
// operations you can perform on objects so you can become comfortable applying
...
console.error("Line 709 // Basic Data Structures: Use the delete Keyword to Remove Object Properties");
...
// the other uses the in keyword. If we have an object users with a property
...
// isEveryoneHere, which we pass the users object as an argument.
// Finish writing this function so that it returns true only if the users
// object contains all four names, Alan, Jeff, Sarah, and Ryan, as keys, and
// false otherwise.
...
console.error("Line 748 // ic Data Structures: Check if an Object has a Property");
...
// looped through the object, resulting in each user's name being
...
// function to loop through the users in the users object and return the number
// of users whose online property is set to true.
...
function countOnline(obj) {
  // change code below this line
  let result = 0;
  for (let userList in obj){
    if (obj[userList].online === true){
       result++;
    }
  }
  return result;
...
console.warn("Line 801 // Basic Data Structures: Iterate Through the Keys of an Object with a for...in Statement");
console.log(countOnline(userList));
...
console.error("Line 867 // Basic Data Structures: Generate an Array of All Object Keys with Object.keys()");
...
// Additionally, the optional Advanced Data Structures lessons later in
// the curriculum also cover the ES6 Map and Set objects, both of which
// are similar to ordinary objects but provide some additional features.
// Now that you've learned the basics of arrays and objects, you're fully
...
// function addFriend. Finish writing it so that it takes a user object
// and adds the name of the friend argument to the array stored
...
function addFriend(userObj, friend) {
  // change code below this line
  let obj = userObj['data']['friends'];
  obj.push(friend);
  return obj;
  // change code above this line
}

console.warn("Line 907 // Basic Data Structures: Modify an Array Stored in an Object");
